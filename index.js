...
app.post("/here", async (req, res) => {
  ...
  /*
  INSERT INTO route_contained_points (uag_id, route_id)
  select ua.id as uag_id, r.id as route_id from gis.summaries s
  join gis.routes r on s.id=r.summary_id
  join gis.uag ua on ST_CONTAINS(ST_Buffer(r.geom, 20, 'endcap=round join=round'), ua.geom)
  group by ua.id, r.id;
  */
